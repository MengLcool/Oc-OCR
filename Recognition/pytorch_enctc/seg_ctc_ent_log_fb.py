...
#    param target_sizes: size for targets (N)
    ...
    # notice : already compute log_softmax in train code 
    ...
    for index, (target_size, size) in enumerate(zip(target_sizes, sizes)):
        ...
        uni_length = int(uni_rate * (size.data.item() / (target_size.data.item() + 1e-8) + 1))
        uni_length = min(uni_length, Time)
...
def test_seg_ctc(use_mine=True):
    ...
    for i in range(50):
        ...
        glog.info('{}, entropy: {}, cost: {}, len: {}'.format(i, H, cost, type(cost)))
