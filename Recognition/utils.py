...
class CTCLabelConverter(object):
    ...
    def encode(self, text, batch_max_length=25):
        ...
        text = [self.dict.get(char,self.dict['#']) for char in text]

        return (torch.IntTensor(text), torch.IntTensor(length))

    def decode(self, text_index, length, text_prob =None):
        ...
        for l in length:
            ...
            if text_prob is not None :
                t_prob = text_prob[index: index+ l]
            char_list = []
            for i in range(l):
                if t[i] != 0 and (not (i > 0 and t[i - 1] == t[i])):  # removing repeated characters and blank.
                    char_list.append(self.character[t[i]])
                    if text_prob is not None :
                        prob *= t_prob[i].item() # avoid memory leak
            text = ''.join(char_list)
            probs.append(prob)
            texts.append(text)
            index += l
